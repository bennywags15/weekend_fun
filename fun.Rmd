...
glm_fit <- glm(any_nmu_numeric ~ DEM_GENDER, data = eighteen, family = binomial)
summary(glm_fit)

#Age: Significant at 0.05 level. Positive coefficient, so older individuals more likely
glm_fit <- glm(any_nmu_numeric ~ DEM_AGE, data = eighteen, family = binomial)
summary(glm_fit)
...
glm_fit <- glm(any_nmu_numeric ~ COLLEGE_HOUSING, data = eighteen, family = binomial)
summary(glm_fit)
```